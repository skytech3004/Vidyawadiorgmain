...
import { AlertCircle, CheckCircle2, Edit2, Loader2, Plus, Save, Trash2, Users, X } from "lucide-react";
...
export default function AdminStaffPage() {
    ...
    const fetchItems = async () => {
        setLoading(true);
        try {
            const res = await fetch("/api/admin/admin-staff");
            const data = await res.json();
            if (data.success) {
                setItems(data.staff || []);
            }
        } catch {
            setBanner({ type: "error", text: "Failed to load admin staff" });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(isEditing ? `/api/admin/admin-staff/${form._id}` : "/api/admin/admin-staff", {
            ...
            const data = await res.json();
            if (data.success) {
                setBanner({ type: "success", text: isEditing ? "Staff member updated." : "Staff member added." });
                closeModal();
                await fetchItems();
            } else {
                setBanner({ type: "error", text: data.error || "Unable to save staff member" });
            }
        } catch {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/admin/admin-staff/${id}`, { method: "DELETE" });
            const data = await res.json();
            if (data.success) {
                setBanner({ type: "success", text: "Staff member deleted." });
                await fetchItems();
            } else {
                setBanner({ type: "error", text: data.error || "Unable to delete staff member" });
            }
        } catch {
    ...
    return (
        <div className="max-w-6xl mx-auto pb-16">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-3xl md:text-4xl font-black text-oxford uppercase tracking-tight">Admin Staff Management</h1>
                    <p className="text-gray-500 mt-2">Add, edit, and remove admin staff records with their contact details.</p>
                </div>
                <button
                    onClick={() => openModal()}
                    ...
                    <table className="w-full text-left border-collapse min-w-[700px]">
                        <thead>
                            <tr className="bg-gray-50/80 text-[10px] font-black uppercase tracking-widest text-gray-400">
                                ...
                                <th className="p-6">Position / Designation</th>
                            ...
                            {items.length === 0 ? (
                                ...
                                items.map((item) => (
                                    <tr key={item._id} className="hover:bg-gray-50/50 transition-colors">
                                        <td className="p-6 font-bold text-oxford">{item.name}</td>
                                        <td className="p-6 text-gray-600">{item.designation}</td>
                                        <td className="p-6 text-gray-600">{item.contactNumber}</td>
                        ...
                        >
                            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/60">
                                <h2 className="text-xl md:text-2xl font-black text-oxford uppercase tracking-tight">
                                    {form._id ? "Edit Staff Member" : "Add Staff Member"}
                                </h2>
                                <button
                                    type="button"
                                    onClick={closeModal}
                            ...
                            <div className="p-6 md:p-8 grid gap-6">
                                ...
                                <div>
                                    <label className="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Position / Designation</label>
                                    <input
                                        required
                                        type="text"
